...
const SettingsPage: React.FC<SettingsPageProps> = ({ hideTitle, section }) => {
  ...
  useEffect(() => {
    ...
    // 监听下载进度
    const handleProgress = (progress: DownloadBinaryProgress) => {
      setDownloadProgress((prev) => ({
        ...prev,
        [progress.binary === 'yt-dlp' ? 'ytDlp' : 'ffmpeg']: progress.percent,
      }));
    };

    window.electronAPI.onDownloadBinaryProgress(handleProgress);

    return () => {
      window.electronAPI.removeAllListeners('download-binary-progress');
    };
  }, []);
  ...
  const handleDownloadBinary = async (binaryName: BinaryName) => {
    const key = binaryName === 'yt-dlp' ? 'ytDlp' : 'ffmpeg';
    setDownloading((prev) => ({ ...prev, [key]: true }));
    setDownloadProgress((prev) => ({ ...prev, [key]: 0 }));

    try {
      const result = await window.electronAPI.downloadBinary(binaryName);
      if (result.success) {
        toast.success(`${binaryName} 下载成功！`);
        await checkBinaries();
      } else {
        toast.error(`下载失败: ${'error' in result ? result.error : '未知错误'}`);
      }
    } catch (error: any) {
                ...
                <div className="flex flex-col justify-between gap-2 rounded-lg border p-4">
                  ...
                  {!binaryStatus?.ytDlp && (
                    <Button
                      ...
                      disabled={downloading.ytDlp}
                      onClick={() => handleDownloadBinary('yt-dlp')}
                    >
                      <Download className={cn("h-4 w-4", downloading.ytDlp && "animate-pulse")} />
                ...
                <div className="flex flex-col justify-between gap-2 rounded-lg border p-4">
                  ...
                  {!binaryStatus?.ffmpeg && (
                    <Button
                      ...
                      variant="outline"
                      className="mt-2 gap-2"
                      disabled={downloading.ffmpeg}
                      onClick={() => handleDownloadBinary('ffmpeg')}
                    >
