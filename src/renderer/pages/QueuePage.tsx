...
const QueuePage: React.FC = () => {
  ...
  const scheduleDownloads = useCallback(() => {
    ...
    try {
      while (activeIds.size < MAX_CONCURRENT_DOWNLOADS && pendingTasks.length > 0 && !queuePaused) {
        ...
        // 防止竞态条件：检查任务是否已经在活跃下载中
        // 这可以防止快速多次调用 scheduleDownloads 时同一任务被启动多次
  ...
  useEffect(() => {
    ...
    // 清理函数：移除所有监听器
    // 注意：removeAllListeners 会移除该频道的所有监听器
    // 由于 QueuePage 是唯一注册这些监听器的组件，这是安全的
    return () => {
